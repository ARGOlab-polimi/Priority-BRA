function [model, constraint] = bicycle_model2(list_constr, sim_data)
import casadi.*
...
% alla fine si lavorerà sempre con umano e ego, i valori giusti rispetto a
% alta priorità o meno vengono scelti durante la simulazione
...
p = vertcat(p, onoff_e, onoff_h, onoff_lane, p5_e, p6_e, p5_h, p6_h, bhj_e, bhj_h, HP, lane_parameter);
...
%     y_left_expr = y+D*sin(alpha+abs(theta))-(y_lim/4);
%     C_left = exp((y_left_expr)); 
%     C_vec = [C_vec; C_left];
% end
% 
% if list_constr.right_lane
%     y_right_expr = y-D*sin(alpha+abs(theta))-(-y_lim/4);
%     C_right = exp((-y_right_expr));
%     C_vec = [C_vec; C_right];
% end

if list_constr.left_lane
    y_left_expr = y+D1/2*sin(theta)-(y_lim/4);
    C_left = exp((y_left_expr)); 
    C_vec = [C_vec; C_left];
end

if list_constr.right_lane
    y_right_expr = y+D1/2*sin(theta)+(y_lim/4);
    C_right = exp((-y_right_expr));
    C_vec = [C_vec; C_right];
end
% if list_constr.left_lane
%     y_left_expr = y - y_lim/4;
%     C_left = exp(y_left_expr); 
%     C_vec = [C_vec; C_left];
% end
% 
% if list_constr.right_lane
%     y_right_expr = y + y_lim/4;
%     C_right = exp(-y_right_expr);
%     C_vec = [C_vec; C_right];
% end
...
if list_constr.center_lane
    ...
    center_lane_eq = 0.01*((1/(1+exp(15*(y+D1*sin(theta)-2))))+(1/(1+exp(-15*(y+D1*sin(theta)+2))))-1);
...
if list_constr.lane_keeping
    ...
    lane_keeping_eq = (y+lane_parameter)^2;
