...
addpath('Results', 'helper_functions_3egos')         

plot_value_functions = 0;   % change to 1 to plot the value functions of all scenarios

ttc_kin = 0;                % 0 to show dynamic ttc, 1 to show kinematic ttc
...
play_scenario = 0;          % change to 1-9 (2 and 8 excluded) to play the corresponding scenario's video

play_ttc_frame = 0;         % change to 1-9 (2 and 8 excluded) to select the corresponding scenario to check visually the ttc value between two vehicles 
id_vehicles = [1 0];        % change to 0-3 to select the two vehicles, 1 for ego1, 2 for ego2, 3 for ego3, 0 for hdv
target_time = 11.5;         % time of the simulation of which i want to see the ttc configuration and evolution
...
plot_ttc_diagrams_mpc = 0;

mpc_trajectory = 0;
id_vehicles_mpc = [1 0];
save_video_mpc = 0;
video_filename_mpc = 'scenario9_ttc_mpc';


%Load all results
res1 = load("Results\scenario1.mat");
res1 = res1.res;
results{1} = res1;
...
res = load("Results\scenario4.mat");
res4 = res.res;
results{3} = res4;
...
ttc = load("Results\ttc_mpc_data_1.mat");
ttc_mpc{1} = ttc.ttc_mpc_data_1.ttc_mpc;
ttc = load("Results\ttc_mpc_data_3.mat");
ttc_mpc{2} = ttc.ttc_mpc_data_3.ttc_mpc;
ttc = load("Results\ttc_mpc_data_4.mat");
ttc_mpc{3} = ttc.ttc_mpc_data_4.ttc_mpc;
ttc = load("Results\ttc_mpc_data_5.mat");
ttc_mpc{4} = ttc.ttc_mpc_data_5.ttc_mpc;
ttc = load("Results\ttc_mpc_data_6.mat");
ttc_mpc{5} = ttc.ttc_mpc_data_6.ttc_mpc;
ttc = load("Results\ttc_mpc_data_7.mat");
ttc_mpc{6} = ttc.ttc_mpc_data_7.ttc_mpc;
ttc = load("Results\ttc_mpc_data_9.mat");
ttc_mpc{7} = ttc.ttc_mpc_data_9.ttc_mpc;

ttc = load("Results\ttc_mpc_data_1.mat");
ttc_mpc_data{1} = ttc.ttc_mpc_data_1;
ttc = load("Results\ttc_mpc_data_3.mat");
ttc_mpc_data{2} = ttc.ttc_mpc_data_3;
ttc = load("Results\ttc_mpc_data_4.mat");
ttc_mpc_data{3} = ttc.ttc_mpc_data_4;
ttc = load("Results\ttc_mpc_data_5.mat");
ttc_mpc_data{4} = ttc.ttc_mpc_data_5;
ttc = load("Results\ttc_mpc_data_6.mat");
ttc_mpc_data{5} = ttc.ttc_mpc_data_6;
ttc = load("Results\ttc_mpc_data_7.mat");
ttc_mpc_data{6} = ttc.ttc_mpc_data_7;
ttc = load("Results\ttc_mpc_data_9.mat");
ttc_mpc_data{7} = ttc.ttc_mpc_data_9;
...
if play_ttc_frame ~= 0
    ...
    ttcs_s = ttc_standard{play_ttc_frame};
    ttcs = ttc_mpc{play_ttc_frame};
    % ev_states = results{play_ttc_frame}.ttc_dyn_data.ev_states_dyn;
    ev_states = ttc_mpc_data{play_ttc_frame}.ev_states_mpc;
    % time_future = results{play_ttc_frame}.ttc_dyn_data.time_future_dyn;
    time_future = ttc_mpc_data{play_ttc_frame}.time_future_mpc;
    Nsim = length(results{play_ttc_frame}.t);
    simX = results{play_ttc_frame}.simX;
    simX_hdv = results{play_ttc_frame}.simX_hdv;
    dt = 3/60;
    model.params.D1 = 4.3;
    model.params.D2 = 1.8;
    ...
    %check_ttc(simX, simX_hdv, target_time, dt, lane_width, id_vehicles, ttc);
    %checkTTC_dyn(ttcs, ev_states, dt, target_time, id_vehicles, simX, time_future)
    plot_frames_ttc(ev_states, target_time, ttcs, id_vehicles, simX, simX_hdv, time_future)
    plot_kinematic_ttc_frames(simX, simX_hdv, ttcs_s, id_vehicles, target_time)
    plot_trajectory_comparison(simX, simX_hdv, ev_states, id_vehicles, target_time)
end
...
if save_frame ~= 0
    ...
    simX = results{save_frame}.simX;
    simX_hdv = results{save_frame}.simX_hdv;
    dt = results{save_frame}.t(2) - results{save_frame}.t(1);
    create_simulation_frame(simX, simX_hdv, target_time_sim, dt, 8);
end

if save_vf ~= 0
    if save_vf == 2 || save_vf > 9
        error('The scenarios indexes are from 1 to 9, 2 is neglected.')
    end
    if save_vf > 1
        save_vf = save_vf-1;
    ...
    V_ego_vs_human = results{save_vf}.V_ego_vs_human;
    V_ego_vs_ego = results{save_vf}.V_ego_vs_ego;
    brt_activated = results{save_vf}.brt_activated;
    Tf =  results{save_vf}.t(end);
    Nsim = length(results{save_vf}.t);
    num_egos = 3; 
    handles = plot_all_value_functions(V_ego_vs_human, V_ego_vs_ego, brt_activated, Tf, Nsim, num_egos);
    end


if plot_ttc_diagrams_mpc
    for i=1:7
        ttcs = ttc_mpc{i};
        t = results{i}.t;
        figure
        ...
        xlabel('Simulation Time [s]')
        ylabel('TTC [s]')
        [min_val, min_idx] = min(ttcs(:,1));
        txt_str = sprintf('min ttc value %.2f s at %.2f s', min_val, t(min_idx));
        text(0.95, 0.95, txt_str, 'Units', 'normalized', 'HorizontalAlignment', 'right', 'VerticalAlignment', 'top');
...
if mpc_trajectory ~= 0
    if mpc_trajectory == 2 || mpc_trajectory > 9
        error('The scenarios indexes are from 1 to 9, 2 is neglected.')
    end
    if mpc_trajectory > 1
        mpc_trajectory = mpc_trajectory-1;
    end
    if mpc_trajectory > 7
        mpc_trajectory = mpc_trajectory-1;
    end
    simX = results{mpc_trajectory}.simX;
    simX_hdv = results{mpc_trajectory}.simX_hdv;
    dt = results{mpc_trajectory}.t(2) - results{mpc_trajectory}.t(1);
    ttc_mpc_data = ttc_mpc_data{mpc_trajectory};
    ttcs = ttc_mpc{mpc_trajectory};
    mpc_ttc_video(simX, simX_hdv, ttcs, ttc_mpc_data, id_vehicles_mpc, dt, video_filename_mpc, save_video_mpc)
end
