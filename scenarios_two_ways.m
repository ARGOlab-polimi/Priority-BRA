...
% IC 
% ego{1}.x0 = [0, -2, 0, 0, 6];    %scenario 3 (Third on the paper)
...
ego{3}.follow_ref = true;

% HDV
N_first_step = 3;
human = {hdv(hdv_x0, Nsim, false)};
brt_hp1 = load('BRT data/BRT_H_temp14.mat'); 
brt_hp1 = brt_hp1.BRT;
brt_hp2 = load('BRT data/BRT_H_temp19.mat'); 
brt_hp2 = brt_hp2.BRT;
brt_lp = load('BRT data/BRT_L_temp42.mat');  
brt_lp1 = brt_lp.BRT;
brt_lp = load('BRT data/BRT_L_temp46.mat'); 
brt_lp2 = brt_lp.BRT;
brt_lp = load('BRT data/BRT_L_temp48.mat'); 
brt_lp3 = brt_lp.BRT;
...
constr_list.lane_keeping = false;
...
for i_solver = 1:num_egos
    ...
    [model, constraint] = bicycle_model3(track_file, constr_list, sim_data);
    ...
    lh(15)     = 0;      % Limit for brt vs ego
    lh(16)     = 0;      % Limit for brt vs hum
...
simX = cell(1, num_egos); 
simU = cell(1, num_egos); 
...
ego{num_egos+1}.x0 = [inf, inf, inf, inf, inf]; % [s, y, theta, delta, v]
...
for i = 1:Nsim
    ...
    for ego_idx = 1:num_egos
        ...
        if ego{ego_idx}.x_current(3) >= pi/2 || ego{ego_idx}.x_current(3) <= -pi/2
            ego_front = ego_obs(2,:);
            ego_back = ego_obs(1,:);
        else
            ego_front = ego_obs(1,:);
        ...
        obstacles = [ego{ego_front(ego_idx)}.x_current([1:3,5])'; human{1}.simX_0'];
        p1 = [p1; obstacles]; 

        HP = 1; 

        if ego{ego_idx}.x0(2) <= 0 
            brt_hp = brt_hp1;
        else
            brt_hp = brt_hp2;
        end
        ...
        brt_activated{ego_idx}(i,1) = onoff_h; % 0=off, 1=HP
        ...
        % evaluate constraint parameters with ego
        [V_e1, onoff_e1, p5_e1, p6_e1, bhj_e1]=evaluateVPBhj(ego{ego_idx}.x_current, u_egos(ego_idx,:), ego{ego_front(ego_idx)}.x_current, u_egos(ego_front(ego_idx),:), model, brt_lp1);
        [V_e2, onoff_e2, p5_e2, p6_e2, bhj_e2]=evaluateVPBhj(ego{ego_idx}.x_current, u_egos(ego_idx,:), ego{ego_front(ego_idx)}.x_current, u_egos(ego_front(ego_idx),:), model, brt_lp3);
        [V_e3, onoff_e3, p5_e3, p6_e3, bhj_e3]=evaluateVPBhj(ego{ego_idx}.x_current, u_egos(ego_idx,:), ego{ego_back(ego_idx)}.x_current, u_egos(ego_back(ego_idx),:), model, brt_lp2);
        
        [~, v_small] = find([V_e1, V_e2, V_e3] == min([V_e1, V_e2, V_e3]));
        switch v_small(1)
            case 1
                V_e = V_e1;
                onoff_e = onoff_e1;
...
do_video = false; 
if do_video
    video_filename = 'scenario_3_POV2.mp4'; 
...
indicator_e_e = cell(num_egos, num_egos); % indicator_e_e{ego_idx, other_ego_idx}

for i_ego = 1:num_egos
    base_y = (num_egos - i_ego + 1) * 1.5; % Posizione Y per questo rigo di indicatori
    ...
    for j_ego = 1:num_egos
        if i_ego ~= j_ego
            ...
            current_x_pos = current_x_pos + 1.8; % Spazia gli indicatori
        end
    end
end
...
for i = 1:Nsim
    ...
    for ego_idx = 1:num_egos
        ...
        if status_e == 1 % LP for ego vs ego
